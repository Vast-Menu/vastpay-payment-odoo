...
// Half a SAR cent: VastPay/this POS is locked to SAR (2 decimals), so any
// real order-content change moves the total by at least 0.01. A fixed
// tolerance avoids depending on the order/currency rounding API, which
// differs between Odoo 18 and 19.
    ...
    async _vastpaySyncConfirmedPayment(invoiceId) {
        ...
        for (const order of orders) {
            for (const line of order.payment_ids || []) {
                ...
                if (["done"].includes(line.get_payment_status?.())) {
                    continue;
                }
                let resp;
                try {
    ...
    _vastpayVoidStaleLines(order, { requireSnapshot = true } = {}) {
        ...
        for (const line of order.payment_ids || []) {
            ...
            if (requireSnapshot) {
                ...
                if (Math.abs(snap - total) < TOTAL_EPSILON) {
